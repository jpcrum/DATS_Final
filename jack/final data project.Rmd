...
While there appears to be a relationship between electrical type and sales price, FP and M only have 8 and 1 observations, respectively. 
...
ggplot(df, aes(x = factor(electrical), y = saleprice)) + geom_boxplot(aes(fill = factor(electrical)))
```
...
summary(lm(saleprice ~ x1st.flr.sf, data = df))
```
...
There doesn't seem to much change in sales price between houses with and without second floors.
...
```{r Garage Exploratory Analysis}

#install.packages("polycor")
library(polycor)

#Selecting garage variables and salepirce
garage.vars <- select(df, garage.type:garage.cond, saleprice)

#Converting categorical variables to factors
garage.vars$garage.type <- as.factor(garage.vars$garage.type)
garage.vars$garage.yr.blt <- as.factor(garage.vars$garage.yr.blt)
garage.vars$garage.finish <- as.factor(garage.vars$garage.finish)
garage.vars$garage.qual <- as.factor(garage.vars$garage.qual)
garage.vars$garage.cond <- as.factor(garage.vars$garage.cond)
colSums(is.na(garage.vars))

#Releveling the factor levels as needed
garage.vars$garage.finish <- factor(garage.vars$garage.finish, levels = c("None", "Unf", "RFn", "Fin"))
garage.vars$garage.qual <- factor(garage.vars$garage.qual, levels = c("None", "Po", "Fa", "TA", "Gd", "Ex"))
garage.vars$garage.cond <- factor(garage.vars$garage.cond, levels = c("None", "Po", "Fa", "TA", "Gd", "Ex"))
```


```{r Garage Graphs}
#garage year built vs sale price, bar graph
ggplot(df, aes(x = garage.yr.blt, y = saleprice, fill = as.factor(garage.yr.blt))) + geom_bar(stat='identity') + theme(legend.position = 'none')

#garage year built vs sale price, histogram
ggplot(df, aes(x = garage.yr.blt, fill = as.factor(garage.yr.blt))) + geom_histogram(stat='count', binwidth = 10) + theme(legend.position = 'none')
```
